...
def build_linear(bondlengths):
    ...
    for idx in range(N-1):
        d = 0.5 * (bondlengths[idx] + bondlengths[idx+1])
        ds.append(d)

    z0 = [0.]
    zs = np.concatenate((z0,ds))

    zchain = np.cumsum(zs)
    zchain -= np.mean(zchain)

    x = np.array([[0.,0.,z] for z in zchain])
...
def random_placement(box,xs_others,ntries=10000):
    ntry = 0
    while True: # random placement
        ntry += 1
        if ntry > ntries:
            raise ValueError(f"Tried {ntries}x to add molecule. Giving up.")
        x0 = build.draw_starting_vec(box) # random point in box
        xs = x0 + xprot # shift geometry to random point
        walls = build.check_walls(xs,box) # check if outside box
        if walls:
            continue
        clash = build.check_clash(xs,xs_others,box) # check if clashes with existing pos
        if clash:
            continue
        else:
            break
